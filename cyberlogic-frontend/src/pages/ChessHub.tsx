...
export default function ChessHub() {
  ...
  return (
    <div className="space-y-6 text-[var(--cl-text-primary)] font-sans relative">
      {/* Header Banner */}
      <div className="bg-gradient-to-r from-[var(--cl-surface-900)] via-[var(--cl-surface-800)] to-[var(--cl-surface-900)] border border-[var(--cl-border)] rounded-2xl p-4 sm:p-6 md:p-8 shadow-2xl relative overflow-hidden">
        <div className="absolute top-0 right-0 -mt-12 -mr-12 w-64 h-64 bg-[var(--cl-primary-glow)] rounded-full blur-3xl pointer-events-none"></div>
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 relative z-10">
          <div>
            ...
            <h1 className="text-2xl sm:text-3xl md:text-4xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-[var(--cl-text-primary)] via-[var(--cl-primary-light)] to-[var(--cl-accent-light)]">
              CyberLogic Chess Hub
            </h1>
            <p className="text-[var(--cl-text-secondary)] mt-1.5 max-w-xl text-xs sm:text-sm md:text-base leading-relaxed">
          ...
          <div className="flex flex-col sm:flex-row flex-wrap items-stretch sm:items-center gap-3.5 w-full lg:w-auto">
            {userStat && (
              <div className="bg-[var(--cl-surface-900)]/90 backdrop-blur border border-[var(--cl-border)] rounded-xl px-4 py-2.5 flex items-center justify-around sm:justify-center gap-5 w-full sm:w-auto">
                <div className="text-center">
                  <div className="text-[11px] text-[var(--cl-text-muted)] font-medium uppercase tracking-wider flex items-center justify-center gap-1">
                    <Trophy className="w-3.5 h-3.5 text-amber-400" /> ELO Rating
                  </div>
                  <div className="text-lg sm:text-xl font-extrabold text-amber-400 mt-0.5">{userStat.elo_rating}</div>
                </div>
                <div className="h-8 w-px bg-[var(--cl-border)]"></div>
                <div className="text-center">
                  <div className="text-[11px] text-[var(--cl-text-muted)] font-medium uppercase tracking-wider flex items-center justify-center gap-1">
                    <Award className="w-3.5 h-3.5 text-emerald-400" /> Reputation
                  </div>
            ...
            <button
              ...
              className="bg-[var(--cl-primary)] hover:brightness-110 active:scale-95 text-slate-950 font-bold px-5 py-3 rounded-xl shadow-lg shadow-[var(--cl-primary-glow)] flex items-center justify-center gap-2 transition-all cursor-pointer w-full sm:w-auto text-sm"
            >
      ...
      <div className="flex items-center border-b border-[var(--cl-border)] overflow-x-auto no-scrollbar">
        <button
      ...
      <div>
        {activeTab === 'lobby' ? (
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            {/* Left Column: Active & Waiting Match Rooms */}
            <div className="lg:col-span-2 space-y-6">
              <div className="bg-[var(--cl-surface-900)] border border-[var(--cl-border)] rounded-2xl p-5 shadow-xl">
                <div className="flex items-center justify-between mb-4">
                  <h2 className="text-lg font-bold flex items-center gap-2 text-[var(--cl-text-primary)]">
                    <Radio className="w-5 h-5 text-emerald-400 animate-pulse" /> Available Match Rooms
                  </h2>
                  <span className="text-xs text-[var(--cl-text-muted)] bg-[var(--cl-surface-800)] px-2.5 py-1 rounded-full border border-[var(--cl-border)]">
                    {games.length} active
                  </span>
                </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {games.map((g) => {
                      ...
                      return (
                        <div
                          key={g.id}
                          className="bg-[var(--cl-surface-950)] border border-[var(--cl-border)] hover:border-[var(--cl-primary)]/50 rounded-xl p-4 transition-all flex flex-col justify-between"
                        >
                          <div>
            ...
            {/* Right Column (Desktop View): Online Players & Lobby Chat Hub */}
            <div className="hidden lg:block space-y-6">
              {/* Online Players Card */}
              <div className="glass border border-[var(--cl-border)] rounded-2xl p-5 shadow-xl">
                <div className="flex items-center justify-between mb-4">
                  <h2 className="text-base font-bold text-[var(--cl-text-primary)] font-[family-name:var(--font-heading)] flex items-center gap-2">
                    <Users className="w-4 h-4 text-[var(--cl-primary)]" /> Online Club Players
                  </h2>
                  <span className="px-2.5 py-0.5 rounded-full text-xs font-semibold bg-[var(--cl-primary-glow)] text-[var(--cl-primary-light)] border border-[var(--cl-primary)]/30">
                ...
                <div className="max-h-56 overflow-y-auto space-y-2.5 pr-1 custom-scrollbar">
                  {onlineUsers.length === 0 ? (
                    ...
                    onlineUsers.map((u) => (
                      <div
                        key={u.id}
                        className="flex items-center justify-between p-2.5 rounded-xl bg-[var(--cl-surface-950)]/70 hover:bg-[var(--cl-surface-950)] border border-[var(--cl-border)]/60 text-xs transition-all"
                      >
                        <div className="flex items-center gap-3">
                          <div className="relative">
                            <img
                              ...
                              alt={u.name}
                              className="w-8 h-8 rounded-full object-cover border border-[var(--cl-border)]/80"
                            />
                            <span
                        ...
                        {user && user.id !== u.id && (
                          <button
                            ...
                            className="bg-[var(--cl-primary)]/15 hover:bg-[var(--cl-primary)]/30 text-[var(--cl-primary-light)] border border-[var(--cl-primary)]/30 rounded-lg text-xs font-semibold px-3 py-1 flex items-center gap-1.5 transition-all cursor-pointer"
                          >
              ...
              <div className="glass border border-[var(--cl-border)] rounded-2xl p-5 shadow-xl flex flex-col h-[420px]">
                <div className="flex items-center justify-between mb-3">
                  <h2 className="text-base font-bold text-[var(--cl-text-primary)] font-[family-name:var(--font-heading)] flex items-center gap-2">
                    <Globe className="w-4 h-4 text-[var(--cl-primary)]" /> Lobby Message Hub
                  </h2>
                  <span className="px-2.5 py-0.5 rounded-full text-[10px] font-mono text-[var(--cl-text-muted)] bg-[var(--cl-surface-950)] border border-[var(--cl-border)]">
                    7-day retention
                  </span>
                </div>
                ...
                <div
                  ...
                  className="flex-1 overflow-y-auto space-y-2.5 mb-3 pr-1 text-xs custom-scrollbar relative"
                >
                  {loadingMoreMessages && (
                    <div className="py-2 text-center text-[var(--cl-primary)] text-[10px] flex items-center justify-center gap-1">
                      <Loader2 className="w-3.5 h-3.5 animate-spin" /> Loading older messages...
                    </div>
                  )}

                  {messages.length === 0 ? (
                    <div className="text-[var(--cl-text-muted)] text-center py-16 text-xs italic">
                      No messages yet. Say hello in the chess lobby!
                    </div>
                  ) : (
                    messages.map((msg, i) => (
                      <div key={msg.id || i} className="bg-[var(--cl-surface-950)]/80 border border-[var(--cl-border)]/60 hover:border-[var(--cl-primary)]/30 p-3 rounded-xl transition-all">
                        <div className="flex items-center justify-between text-[10px] text-[var(--cl-text-muted)] mb-1">
                          <span className="font-bold text-[var(--cl-primary-light)]">{msg.sender?.name || 'Player'}</span>
                          <span>
                            {msg.created_at ? new Date(msg.created_at).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) : ''}
                          </span>
                        </div>
                        <p className="text-[var(--cl-text-primary)] break-words leading-relaxed text-xs sm:text-sm font-normal">{msg.text}</p>
                      </div>
                    ))
                  )}
                </div>
                ...
                <form onSubmit={handleSendChat} className="flex gap-2">
                  <input
                    ...
                    className="flex-1 bg-[var(--cl-surface-950)] border border-[var(--cl-border)] text-xs text-[var(--cl-text-primary)] rounded-xl px-4 py-2.5 focus:outline-none focus:border-[var(--cl-primary)] transition-all"
                  />
                  <button
                    type="submit"
                    className="bg-[var(--cl-primary)] text-slate-950 font-bold p-2.5 rounded-xl transition-all hover:brightness-110 cursor-pointer shadow-lg shadow-[var(--cl-primary-glow)]"
                  >
          ...
          <div className="bg-[var(--cl-surface-900)] border border-[var(--cl-border)] rounded-2xl p-6 shadow-xl">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
              <div>
                <h2 className="text-xl font-bold text-[var(--cl-text-primary)] flex items-center gap-2">
              ...
              <div className="flex items-center gap-2 bg-[var(--cl-surface-950)] p-1 rounded-xl border border-[var(--cl-border)]">
                <button
              ...
              <table className="w-full text-left text-sm text-[var(--cl-text-primary)]">
                <thead className="bg-[var(--cl-surface-950)] text-[var(--cl-text-muted)] uppercase text-[11px] tracking-wider font-semibold border-b border-[var(--cl-border)]">
                  <tr>
                    ...
                    <th className="px-4 py-3">Peak ELO</th>
                    <th className="px-4 py-3">Ranked Record (W/L/D)</th>
                    <th className="px-4 py-3">Reputation Points</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-[var(--cl-border)]">
                  {leaderboard.length === 0 ? (
                    <tr>
                      <td colSpan={6} className="py-8 text-center text-[var(--cl-text-muted)]">
                    ...
                    leaderboard.map((stat, index) => {
                      ...
                      return (
                        <tr key={stat.id} className="hover:bg-[var(--cl-surface-800)]/40 transition-colors">
                          <td className="px-4 py-3 font-bold text-[var(--cl-text-muted)]">
                            {rank === 1 ? (
                          ...
                          <td className="px-4 py-3">
                            <div className="flex items-center gap-3">
                              <img
                                ...
                                alt="avatar"
                                className="w-8 h-8 rounded-full border border-[var(--cl-border)] object-cover"
                              />
                              <div>
                                <div className="font-semibold text-[var(--cl-text-primary)]">
                          ...
                          <td className="px-4 py-3 font-extrabold text-amber-400 font-mono text-base">
                            {stat.elo_rating}
                          </td>
                          <td className="px-4 py-3 text-[var(--cl-text-muted)] font-mono text-xs">{stat.peak_elo}</td>
                          <td className="px-4 py-3 text-xs font-mono text-[var(--cl-text-secondary)]">
                ...
                <div className="flex flex-col h-full">
                  ...
                  >
                    ...
                    {messages.length === 0 ? (
                      ...
                      messages.map((msg, i) => (
                        <div key={msg.id || i} className="bg-[var(--cl-surface-950)] border border-[var(--cl-border)] p-2.5 rounded-lg">
                          <div className="flex items-center justify-between text-[10px] text-[var(--cl-text-muted)] mb-1">
                          ...
                          <p className="text-[var(--cl-text-primary)] break-words leading-relaxed">{msg.text}</p>
                        </div>
                      ))
                    )}
                  </div>
