...
import { MessageSquare, Loader2, Send, Clock, ChevronDown, ChevronUp } from 'lucide-react';
...
export function LobbyMessageHubCard({
  ...
  const [isMinimized, setIsMinimized] = useState(() => {
    return localStorage.getItem('chess_lobby_chat_minimized') === 'true';
  });

  const toggleMinimize = () => {
    setIsMinimized((prev) => {
      const next = !prev;
      localStorage.setItem('chess_lobby_chat_minimized', String(next));
      return next;
    });
  };

  return (
    <div className={`bg-[var(--cl-surface-900)]/90 backdrop-blur-md border border-[var(--cl-border)] rounded-2xl p-4 flex flex-col overflow-hidden transition-all duration-300 ${
      isMinimized ? 'max-h-[58px] min-h-0 flex-none' : 'flex-1 min-h-[240px]'
    }`}>
      {/* Panel Header */}
      <div className="flex items-center justify-between gap-2 shrink-0 pb-2.5 border-b border-[var(--cl-border)]/60">
        ...
        <div className="flex items-center gap-2 shrink-0">
          <span className="px-2 py-0.5 rounded-full text-[10px] font-medium text-[var(--cl-text-muted)] bg-[var(--cl-surface-950)] border border-[var(--cl-border)] flex items-center gap-1">
          ...
          <button
            ...
            title={isMinimized ? 'Expand Arena Lobby Chat' : 'Minimize Panel'}
          >
      ...
      {!isMinimized && (
        <>
          ...
          <div
            ...
            className="flex-1 overflow-y-auto space-y-2 my-3 pr-1 text-xs custom-scrollbar relative min-h-0"
          >
        {loadingMoreMessages && (
